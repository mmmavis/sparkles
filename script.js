...
const FIREBASE_CONFIG = generateFirebaseConfig(`AIzaSyBvp8I4B_57syWZ6BoFI9KxeZDagT75dcI`, `youri-sparkles`);
    ...
    setTimeout(function () {
      renderAllNotes(function() {
        setTimeout(function() {
          ...
          $("#inner-wrapper").append("<div>"+createHTML({},"new-note-template")+"</div>");
          addGlitters();
        }, ANIMATION_DELAY);
      });
    }, 1500);
...
function renderAllNotes(done) {
  firebase.database().ref(FIREBASE_CONFIG.DB_PATH).once('value').then(function(snapshot) {
...
function renderNote(note) {
  ...
  note.gemImage = "assets/gem/" + note.gemType + ".svg";
